...
const MenuRadialPositioner: React.FC<MenuRadialPositionerProps> = ({ children, show = false, style, elipsis, radius = 100 }) => {
  ...
  const calcY = (i: number) => {
    ...
    return sign * (_elipsis.a * _elipsis.b) / Math.sqrt(_elipsis.a ** 2 + ((_elipsis.b ** 2) / (Math.tan(deg) ** 2)));
  }

  const calcX = (i: number) => {
    const deg = degToRad((i + 1) * shiftedDegs);
    const sign = deg > -Math.PI / 2 && deg < Math.PI / 2 ? 1 : -1
    return sign * (_elipsis.a * _elipsis.b) / Math.sqrt(_elipsis.b ** 2 + ((_elipsis.a ** 2) * (Math.tan(deg) ** 2)));
